...
//_____________________________________________________________________________
BeamMagnetQuadrupole_cone::BeamMagnetQuadrupole_cone(G4String nam, G4double xpos,G4double ypos,G4double zpos,G4double length, G4double r1, G4double r2, G4double dout, G4double angle, G4double gradient,G4double shiftX,G4double rx,G4double ry,G4double conerotation, G4LogicalVolume *top):
	Detector(), G4VSensitiveDetector(nam), fNam(nam) {
		...
		coneRot->rotateY(conerotation*1e-3*rad);

		G4String nam_inner = fNam+"_inner";
		G4String nam_cone = fNam+"_cone";
		G4Cons *shape_inner = new G4Cons(nam_inner, 0, r2, 0, r1, length/2, 0, 360*deg);
		G4Cons *shape_cone = new G4Cons(nam_cone, 0, rx, 0, rx, length/2+0.01*meter, 0, 360*deg);

		G4Material *mat_inner = G4NistManager::Instance()->FindOrBuildMaterial("G4_Galactic");
		G4Material *mat_cone = G4NistManager::Instance()->FindOrBuildMaterial("G4_Galactic");

		G4LogicalVolume *vol_inner = new G4LogicalVolume(shape_inner, mat_inner, nam_inner);
		G4LogicalVolume *vol_cone = new G4LogicalVolume(shape_cone, mat_cone, nam_cone);
		vol_inner->SetVisAttributes( G4VisAttributes::GetInvisible() );
		vol_cone->SetVisAttributes( G4VisAttributes::GetInvisible() );
		...
		G4SubtractionSolid *shape_vessel_1 = new G4SubtractionSolid(fNam+"_1", shape_outer, shape_inner);
		G4SubtractionSolid *shape_vessel = new G4SubtractionSolid(fNam, shape_vessel_1, shape_cone,coneRot,G4ThreeVector(shiftX,0,0));
		...
		vis_vessel->SetColor(1, 0, 0); // blue
	...
	}//BeamMagnetQuadrupole_cone

//_____________________________________________________________________________
G4bool BeamMagnetQuadrupole_cone::ProcessHits(G4Step *step, G4TouchableHistory*) {
