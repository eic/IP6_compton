...
//_____________________________________________________________________________
photonDet::photonDet(const G4String& nam, G4double x, G4double y, G4double z, G4double rotation, G4double xysize, G4LogicalVolume *top): Detector(),
  G4VSensitiveDetector(nam), fNam(nam) {

  G4cout << "  photonDet: " << fNam << G4endl;

  //detector shape
  //G4double xysize = 2000*mm;
  G4double zsiz = 1*mm;
  G4Box *shape = new G4Box(fNam, xysize/2., xysize/2., zsiz/2.);
  ...
  fSelectPrim = true;
  //G4cout << "  photonDet, select_prim: " << fSelectPrim << G4endl;
...
//  G4cout<<" Hits X: "<<fHx<<" Hits Y: "<<fHy<<" Hits Z: "<<fHz<<G4endl;
  //G4cout << "photonDet::ProcessHits: " << track->GetParentID() << G4endl;
...
//_____________________________________________________________________________
void photonDet::CreateOutput(TTree *tree) {

  //output from photonDet
